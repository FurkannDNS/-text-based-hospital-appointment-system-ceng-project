...
class HastaneSistemi:
    ...
    def hasta_kayit_menu(self):
        ...
        if tc == "" or len(tc) != 11 or not tc.isdigit():
            print("11 Haneli TC Girmeniz Zorunludur.")
            return
    
        for i in range(3,0,-1):
            sifre = input("Şifre: ").strip()
            if sifre == "" or len(sifre) < 5 or not any(char.isdigit() for char in sifre):
                print(f"En az 5 Karakter Içinde Sayi Olan Sifre Girin.{i}hakkınız kaldı\n")
                if i==0:
                    print("Hakkınız Doldu. Menuye Dönüyorsunuz")
                    system("cls")
                    break
            else:
